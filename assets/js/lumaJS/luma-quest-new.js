class LumaQuestSystem {
  ...
  setupEventListeners() {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (startBtn) startBtn.addEventListener('click', () => this.startQuest())
  ...
  selectOption(stepIndex, optionId) {
    ...
    document.getElementById('next-btn').disabled = !this.isValidSelection(question.id)
  }
  ...
  viewProgram(programId) {
    // Scroll to program catalog and highlight the program
    const catalogSection = document.getElementById('program-catalog')
    if (catalogSection) {
      catalogSection.scrollIntoView({ behavior: 'smooth' })
      
      // Store highlighted program for catalog to pick up
      localStorage.setItem('highlighted_program', programId)
      
      // Trigger highlight after scroll
      setTimeout(() => {
        if (window.highlightProgram) {
