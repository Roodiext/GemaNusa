// Program Catalog JavaScript - Optimized for Mobile with 2 columns
...
class ProgramCatalog {
  ...
  init() {
    ...
    this.ensureFilterButtonLabels()
    // Setel tampilan aktif pada tombol filter sesuai kategori saat ini
  ...
  injectUIFixes() {
    ...
    style.innerHTML = `
      /* CRITICAL: Force filter button and children to always be visible */
      .filter-btn {
      ...
      /* Program cards - consistent height and alignment */
      ...
      #programs-grid .program-card > div:last-child {
        flex: 1;
        display: flex;
        flex-direction: column;
      ...
      #programs-grid .program-card img { 
        display: block; 
        width: 100%; 
        height: 100%; 
        object-fit: cover; 
      ...
      .program-detail-overlay.hidden { 
        display: none; 
      }
      
      /* Advanced filter modal proper centering */
      #advanced-filter-modal {
        display: flex !important;
        align-items: center !important;
        justify-content: center !important;
      }
      #advanced-filter-modal.hidden {
        display: none !important;
      }

      /* Tambah jarak bawah catalog agar tidak mepet footer */
      #program-catalog {
      ...
      @media (max-width: 640px) {
        ...
        /* Ensure 2 columns layout works well */
        .mobile-program-grid {
  ...
  ensureFilterButtonLabels() {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
      ...
      if (existing) {
        ...
        btn.innerHTML = `<span class="filter-label inline-flex items-center" style="gap:.3rem;white-space:normal;">${svgHTML}<span class="text-truncate-1">${label}</span></span>`
      }
  ...
  updateAdvancedFilterButton() {
    ...
    if (hasActiveFilters) {
      ...
      if (!advancedBtn.querySelector('.active-indicator')) {
        ...
        indicator.className = 'active-indicator ml-1 bg-white text-orange-500 px-1 py-0.5 rounded-full text-xs font-medium'
  ...
  renderPrograms() {
    ...
    if (filteredPrograms.length === 0) {
      ...
      grid.innerHTML = '<div class="col-span-full text-center py-10 md:py-16"><div class="text-gray-400 mb-3 md:mb-4"><svg class="w-12 h-12 md:w-16 md:h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.29-1.009-5.824-2.562M15 6.5a7.966 7.966 0 00-6-2.5c-1.035 0-2.024.174-2.947.5M3 12a9 9 0 1118 0 9 9 0 01-18 0z"></path></svg></div><h3 class="text-base md:text-xl font-semibold text-gray-600 mb-1 md:mb-2">Tidak ada program yang ditemukan</h3><p class="text-gray-500 text-xs md:text-base">Coba ubah kata kunci pencarian atau filter kategori</p></div>'
    ...
    grid.innerHTML = filteredPrograms.map(([id, program]) => `
      <div class="program-card mobile-program-card bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col" 
           data-program-id="${id}" data-category="${program.category}">
        
        <!-- Image placeholder (pinned at top) -->
        <div class="w-full h-24 md:h-44 overflow-hidden bg-gray-100 flex-shrink-0">
        ...
        <div class="p-2 md:p-6 border-b border-gray-100 flex-shrink-0">
          <div class="flex items-start justify-between mb-1 md:mb-3">
            <div class="flex items-center space-x-1 md:space-x-3">
              <div class="w-2 h-2 md:w-3 md:h-3 rounded-full flex-shrink-0" style="background-color: ${program.color}"></div>
              <span class="text-xs md:text-sm font-medium text-gray-600 px-1 md:px-2 py-0.5 md:py-1 bg-gray-100 rounded text-truncate-1">
                ${this.getCategoryLabel(program.category)}
              </span>
            </div>
            <!-- Commitment time - hidden on mobile -->
            <span class="mobile-commitment text-xs text-gray-500 bg-gray-100 px-1 md:px-2 py-0.5 md:py-1 rounded flex-shrink-0">
              ${this.getCommitmentLabel(program.commitment)}
            </span>
          </div>
          
          <h3 class="mobile-program-title text-sm md:text-lg font-semibold text-gray-900 mb-1 md:mb-2 text-truncate-1">${program.name}</h3>
          <p class="mobile-program-desc text-gray-600 text-xs line-clamp-2">${program.description}</p>
        </div>
        
        <!-- Content - flex grow to push buttons to bottom -->
        <div class="p-2 md:p-6 flex-1 flex flex-col">
          <!-- Info ringkas -->
          <div class="space-y-1 md:space-y-2 mb-2 md:mb-4">
            <div class="flex items-center text-xs md:text-sm text-gray-600">
              ...
              <span class="text-truncate-1">${this.getScopeLabel(program.scope)}</span>
            </div>
            <div class="flex items-center text-xs md:text-sm text-gray-600">
              <svg class="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
              </svg>
              <span class="text-truncate-1">${program.rolesAvailable.map(role => this.getRoleLabel(role)).join(', ')}</span>
            </div>
          </div>
          
          <!-- Impact -->
          <div class="mb-2 md:mb-6 flex-1">
            <p class="text-xs text-gray-500 font-medium mb-1">DAMPAK</p>
            <p class="mobile-impact text-gray-700">${program.impact}</p>
          </div>
          
          <!-- Action buttons - pinned to bottom -->
          <div class="flex flex-col md:flex-row space-y-1 md:space-y-0 md:space-x-3 mt-auto">
            <button class="mobile-join-btn flex-1 py-1 md:py-2.5 px-2 md:px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-sm font-medium rounded transition-colors duration-200" 
                    onclick="window.programCatalog.joinProgram('${id}')">
              Bergabung
            </button>
            <button class="mobile-detail-btn px-2 md:px-4 py-1 md:py-2.5 border border-gray-200 text-gray-700 text-xs md:text-sm font-medium rounded hover:bg-gray-50 transition-colors duration-200" 
...
function initializeProgramCatalog() {
  ...
  setTimeout(() => {
    if (!window.programCatalog) {
      ...
      window.programCatalog.ensureFilterButtonLabels()
    }
  }, 200)
}
  ...
  if (!document.getElementById('programs-grid')) {
    ...
    if (catalogSection && !document.getElementById('programs-grid')) {
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6" id="programs-grid">
